...
if __name__ == '__main__':
    imgdir = r'F:\浏览器下载\DeepCrack\train_img'
    labdir = r'F:\浏览器下载\DeepCrack\train_lab'
...
batchsz = 1
model =Net().to(device)
savedir = r'F:\浏览器下载\trainondeepcrackwithmab.pth'
imgdir = r'F:\浏览器下载\DeepCrack\test_lab'
labdir = r'F:\浏览器下载\DeepCrack\test_img'
imgsz = 512
resultsdir = r'F:\tf-hea-hv\resultfor6'

dataset = Datases_loader(imgdir, labdir, imgsz, imgsz)
testsets = DataLoader(dataset, batch_size=batchsz, shuffle=False)

def test():
    model.load_state_dict(torch.load(savedir, map_location=torch.device('cpu'), weights_only=True))
